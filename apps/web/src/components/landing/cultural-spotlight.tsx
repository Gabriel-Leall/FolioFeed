  ...
  return (
    <section className="py-32 px-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-end mb-24 cursor-default">
        <h2 className="font-body text-4xl font-semibold md:text-6xl text-on-surface">Destaques Culturais</h2>
        <Link href="/feed" className="group flex items-center gap-4 text-sm font-label uppercase tracking-[0.2em] text-primary">
              ...
              >
                <div className="relative h-2/3 w-full overflow-hidden">
                  <Image
                    ...
                    src={portfolio.previewImageUrl || "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=1000&auto=format&fit=crop"}
                  />
                ...
                <CardFooter className="p-8 flex-col items-start justify-between grow rounded-b-xl">
                  <div className="w-full text-left">
                    <span className="text-xs font-label text-primary uppercase tracking-[0.2em] mb-2 block">{portfolio.area || "Objeto Físico"}</span>
                    <h3 className="font-body text-2xl font-semibold text-on-surface line-clamp-2">{portfolio.title}</h3>
                  </div>
                  <div className="flex w-full justify-between items-center pt-6 border-t border-outline-variant/10 mt-auto">
                    <span className="text-on-surface-variant font-body text-sm truncate pr-4">{portfolio.author?.nickname || "Anônimo"}</span>
